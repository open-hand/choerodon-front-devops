...
(function () {
  ...
  const { DIFF_EQUAL } = diff_match_patch;
  const { DIFF_INSERT } = diff_match_patch;
  const { DIFF_DELETE } = diff_match_patch;

  const { Pos } = CodeMirror;
  const svgNS = 'http://www.w3.org/2000/svg';

  function DiffView(mv, type, options) {
    this.mv = mv;
    this.type = type;
    const obj = {
      left: {
    ...
    this.classes = obj[type];
    if (options.customRightClasses) {
      this.classes = options.customRightClasses;
    }
  }
  ...
  function registerUpdate(dv) {
    ...
    dv.edit.on('viewportChange', () => {
      set(false);
    });
    dv.orig.on('viewportChange', () => {
      set(false);
    });
    update();
    return update;
  ...
  var MergeView = (CodeMirror.MergeView = function (node, options) {
    ...
    const { origLeft } = options;
    const origRight = options.origRight == null ? options.orig : options.origRight;
    ...
    if (hasLeft) {
      left = this.left = new DiffView(this, 'left', options);
      var leftPane = elt(
        'div',
        null,
    ...
    if (hasRight) {
      right = this.right = new DiffView(this, 'right', options);
  ...
  function chunkBoundariesAround(chunks, n, nInEdit) {
    let beforeE; let afterE; let beforeO; let
      afterO;
    for (let i = 0; i < chunks.length; i++) {
      const chunk = chunks[i];
  ...
  TrackAlignable.prototype = {
    ...
    find(n) {
      for (let i = 0; i < this.alignable.length; i += 2) if (this.alignable[i] == n) return i;
      return -1;
    },
    ...
    hasWidget(n) {
      const handle = this.cm.getLineHandle(n);
      if (handle.widgets) for (let i = 0; i < handle.widgets.length; i++) if (!handle.widgets[i].above && !handle.widgets[i].mergeSpacer) return true;
      return false;
    },
