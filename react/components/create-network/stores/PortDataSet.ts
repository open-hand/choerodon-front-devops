...
export default ({
   ...
   * @param record
   */
  function checkPort(value: string, type: any, record: { dataSet: any[]; id: any; }) {
    const p = /^([1-9]\d*|0)$/;
    const isRepeat = record.dataSet.findIndex((r: { id: any; get: (arg0: any) => any; }) => record.id !== r.id && r.get(type) === value) !== -1;
        ...
        lookupAxiosConfig: {
          ...
          transformResponse: (resp: string) => {
            try {
              ...
              return map(data, (item: { resourceName: any; portValue: any; }, index: any) => ({
                ...item,
