...
const NetWorkStore = createContext({} as any);
    ...
    if (networkId && formDs.current) {
      ...
      networkInfoDs.query().then(res => {
        ...
        if (!targetAppServiceId && instances && instances.length) {
          forEach(instances, (item:any, index:number) => {
...
function loadInfo({ data, formatMessage, targetLabelsDs, portDs, formDs, networkInfoDs }:any) {
...
function initTargetLabel({ targetLabelsDs, type, record, networkInfoDs, formatMessage }:any) {
  ...
  if (type === 'instance') {
    if (targetAppServiceId) {
      ...
      if (instances.length > 1) {
        return map(instances, (item:any) => item.code).join(',');
      }
    ...
    if (isEmpty(selectors)) {
      ...
      forEach(selectors, (value:any, keyword:any) => (
        targetLabelsDs.create({
          keyword,
          value,
        })
...
function initPorts({ portDs, type, networkInfoDs }:any) {
  const { externalIps: ips, ports }:any = networkInfoDs.current.get('config');
  portDs.reset();
  if (type === networkInfoDs.current.get('type')) {
    forEach(ports, (item: any) => {
      portDs.create(item);
    });
  } else {
    portDs.create();
  ...
  if (type === 'ClusterIP') {
    if (typeof networkInfoDs.current.get('config') === 'string') {
      return [ips];
    } else {
      return ips;
    }
