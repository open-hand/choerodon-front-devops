...
const DynamicSelect = injectIntl(observer((props) => {
  const {
    intl: { formatMessage }, selectDataSet, optionsFilter, optionsRenderer, optionsDataSet, addText, selectName, loadMore, renderer, optionKeyName = selectName,
  } = props;
  ...
  async function loadData(searchPage = 1) {
    const res = optionsDataSet && await optionsDataSet.query(searchPage);
    if (res && !res.failed) {
  ...
  function rendererWraper({ record, text, value }) {
    ...
    if (!renderer) return text;
    return (
      <div className="renderer-wraper">
        {renderer({
          valueRecord: record, text, value, optionRecord: store.optionMap[value],
        })}
      </div>
    );
  }
  ...
  return (
    <div className="dynamic-select-form">
