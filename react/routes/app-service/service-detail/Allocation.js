...
const Allocation = withRouter(
  observer((props) => {
    const { intlPrefix, prefixCls, appServiceStore } = useAppTopStore();
    const {
      intl: { formatMessage },
      permissionDs,
      detailDs,
    ...
    function renderAction({ record }) {
      if (
        (detailDs.current && detailDs.current.get('skipCheckPermission'))
        || record.get('gitlabProjectOwner')
      ) return;
      const actionData = [
        {
    ...
    function openDetail(appServiceIds) {
      ...
      history.push(
        `/rducm/code-lib-management/assign${location.search}&appServiceIds=${appServiceIds}`,
      );
      // Modal.open({
      //   key: modalKey1,
      //   title: <Tips
    ...
    function renderButtons() {
      ...
      return (
        ...
        //     placement="bottom"
        //   >
        <Permission service={['choerodon.code.project.develop.app-service.ps.permission.update']}>
          <Button icon="authority" onClick={() => openDetail(props.match.params.id)}>
    ...
    return (
      <TabPage service={['choerodon.code.project.develop.app-service.ps.permission']}>
        <HeaderButtons>
          {renderButtons()}
