...
function getRequestData(data, res) {
  const {
    url, userName, password, chartType,
  } = data;
  if (chartType === 'custom') {
...
export default (({
  intlPrefix, formatMessage, projectId, appServiceId, dockerDs,
}) => {
  ...
  function checkPom(value, name, record) {
    const pa = /^[A-Za-z0-9_\-.]+$/;
    if (value && !pa.test(value)) {
      return formatMessage({ id: `${intlPrefix}.pom.check.failed` });
    }
    return true;
  }
  ...
  return ({
    ...
    transport: {
      ...
      update: ({ data: [data] }) => {
        const res = pick(data, ['id', 'name', 'chart', 'objectVersionNumber', 'imgUrl', 'harborRepoConfigDTO', 'artifactId', 'groupId']);
    ...
    fields: [
      ...
      {
        name: 'artifactId', label: 'artifactId', validator: checkPom, maxLength: 512,
      },
      {
        name: 'groupId', label: 'groupId', validator: checkPom, maxLength: 512,
      },
    ],
    events: {
      update: handleUpdate,
