...
export default ({ formatMessage, intlPrefix }) => {
  ...
  return ({
    ...
    fields: [
      { name: 'id', type: 'string' },
      { name: 'name', type: 'string', label: formatMessage({ id: 'c7ncd.deployment.application.net.name' }) },
      { name: 'error', type: 'string' },
      { name: 'status', type: 'string' },
      { name: 'config', type: 'object' },
      { name: 'type', type: 'string', label: formatMessage({ id: 'c7ncd.deployment.application.net.configType' }) },
