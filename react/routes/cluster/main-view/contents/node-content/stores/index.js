...
import React, {
  useContext, useMemo, useEffect, createContext,
} from 'react';
...
const NodeContentStoreProvider = injectIntl(
  inject('AppState')(
    observer((props) => {
      const {
        intl: { formatMessage },
      ...
      async function loadMonitor() {
        ...
        if (res) {
          ...
          axios.get('/oauth/is_login', { withCredentials: true }).then((response) => {
            if (!response) {
      ...
      return <NodeContentStore.Provider value={value}>{children}</NodeContentStore.Provider>;
    }),
  ),
);

export { useNodeContentStore, NodeContentStoreProvider };
