...
function IssueDetail(props) {
  ...
  return (
    <div>
      {issue ? (
        <div className="c7n-branch-issue">
          <section className="branch-issue-name">
            <h2 className="issue-num">
              {issue.issueNum}
              {' '}
            </h2>
              ...
              <div className="issue-status">
                ...
                <div>
                  ...
                  <div className="issue-status-text">
                    {issue.storyPoints}
                    {formatMessage({ id: 'branch.issue.story.point_p' })}
                  </div>
                </div>
              </div>
            ) : null}
          ...
          <section className="branch-issue-detail">
            ...
            <div className="issue-detail-content">
              <div className="issue-detail-tr">
                <div>
                  <span className="issue-detail-content-title">
                    {formatMessage({ id: 'branch.issue.module' })}
                    :
                  </span>
                  {issue.componentIssueRelVOList.length ? issue.componentIssueRelVOList.map((module) => (
                    <span key={module.name} className="issue-detail-module-value">
                      {module.name}
                      {`${issue.componentIssueRelVOList.length > 1 ? ',' : ''}`}
                    </span>
                  )) : '无'}
                </div>
                <div>
                  <span className="issue-detail-content-title">
                    {formatMessage({ id: 'branch.issue.label' })}
                    :
                  </span>
                ...
                <div>
                  <span className="issue-detail-content-title">
                    {formatMessage({ id: 'branch.issue.fixVer' })}
                    :
                  </span>
                  {issue.versionIssueRelVOList.length
                    ? issue.versionIssueRelVOList.map((label) => (
                      <span className="issue-detail-module-value">
                        {label.relationType === 'fix' ? label.name : formatMessage({ id: 'branch.issue.no' })}
                        {`${issue.versionIssueRelVOList.length > 1 ? ',' : ''}`}
                      </span>
                    )) : '无'}
                </div>
                <div>
                  <span className="issue-detail-content-title">
                    {formatMessage({ id: 'branch.issue.epic' })}
                    :
                  </span>
                ...
                <div>
                  <span className="issue-detail-content-title">
                    {formatMessage({ id: 'branch.issue.workTime' })}
                    :
                  </span>
              ...
              <div className="issue-detail-tr">
                <div>
                  ...
                  <div>
                    <span className="issue-detail-content-title" style={{ marginLeft: 0 }}>
                      {formatMessage({ id: 'branch.issue.reporter' })}
                      ：
                    </span>
                  ...
                  <div className="issue-detail-content-title" style={{ marginLeft: 0 }}>
                    {formatMessage({ id: 'branch.issue.assignee' })}
                    ：
                  </div>
                  {issue.assigneeImageUrl ? <div className="issue-user-img" style={{ backgroundImage: `url(${issue.assigneeImageUrl})` }} /> : <>{issue.assigneeName ? <div className="issue-user-img">{getName(issue.assigneeName)}</div> : formatMessage({ id: 'branch.issue.no' })}</>}
                ...
                <div>
                  ...
                  <div>
                    <span className="issue-detail-content-title" style={{ marginLeft: 0 }}>
                      {formatMessage({ id: 'branch.issue.createTime' })}
                      ：
                    </span>
                  ...
                  <div>
                    <span className="issue-detail-content-title" style={{ marginLeft: 0 }}>
                      {formatMessage({ id: 'branch.issue.updateTime' })}
                      :
                    </span>
                    <div style={{ display: 'inline-block' }}>
